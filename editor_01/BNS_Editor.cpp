...
bool BNS_Editor::CreateNoteScreenGrid()
{
	...
	auto xLineDraw = [](int y) { y = _noteScn_height - y - 1; WNDRD.SingleLine(_noteScn_offsetWidth, y, _noteScn_offsetWidth + _noteScn_laneWidth * 4, y); };
...
bool BNS_Editor::CreateNoteScreen()
{
	...
	auto _DrawNote = [&](int lane, int beat)
	{
		...
		int y = _noteScn_height - (_noteScn_offsetHeight + (_noteScn_beatHeight * beat) - m_noteCursor + height + 1);
	...
	int y = _noteScn_height - _noteScn_offsetHeight - 1;
...
void BNS_Editor::DrawNoteScreenGrid()
{
	...
	WNDRD.Copy(0, 0, 1600 - _noteScn_width, 0, rt.Width(), rt.Height());
...
void BNS_Editor::DrawNoteScreen()
{
	...
	WNDRD.DrawOnMainBuffer();
	WNDRD.CopyTargetBitmap(rt);
	WNDRD.CopyTransparent(0, 0, 1600 - _noteScn_width, 0, rt.Width(), rt.Height());
	WNDRD.UnlinkCopyTargetBitmap();
}
